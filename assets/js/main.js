$(document).ready(function(){
	...
	if((location.indexOf('index.html')!=-1)||(location=='https://reddy.netlify.com/')){
		reviewsAjax();
		newsAjax();
		topReviewsAjax();
		newsSlider();
	...
	document.getElementById('navSmall').addEventListener('click',showMenu);
...
function proveraSignIn(){
	...
	if(!(regexPassword.test(passwordValue))){
		password.style.borderBottom = "2px solid #cf2525";
		errors.push('<div class="error"><p>Password one uppercase,lowercase letter and number.</p></div>')
		valid = false;
	} else{
		password.style.borderBottom = "2px solid green";
	}
...
function proveraContact(){
	...
	if(!(regexFullName.test(nameValue))){
		...
		errors.push('<div class="error"><p>Name nn invalid format.</p></div>')
		valid = false;
	} else{
	...
	if(!(regexMessage.test(messageValue))){
		...
		errors.push('<div class="error"><p>Message max 250 characters.</p></div>')
		valid = false;
	} else{
		...
		block += `
			<div class="indexSliderMovie">
				...
				<div>
					...
					<p>`;
					if(element.description){ block +=element.description}
					block+= `</p>
				</div>
			</div>
		`
...
function getGenres(){
	getObj('genres.json',function(data){
		writeGenres(data);
	})
}
function writeGenres(data){
	let block= "";
	var genres = [];
	block +=`<option value='0'>Choose Movie</option>`
	broj=0;
	for(element of data){
		broj++;
		block += `<option value="${broj}">${element.name}</option>`;

	}
	document.getElementById('genreSelect').innerHTML = block;	
}
function reviewProvera(){
	document.getElementById('addReviewErrorsBlock').innerHTML = '';
	var errors = [];
	var genreList = document.getElementById("genreSelect"),
		reviewText = document.getElementById("reviewText");
	var genreListValue = genreList.value,
	...
	if(!(regexText.test(reviewTextValue))){
		...
		errors.push("<div class='error'><p>Text between 1 and 500 characters</p></div>")
		valid = false;
	} else{
	...
	if(errors.length>0){
		for(element of errors){
			document.getElementById('addReviewErrorsBlock').innerHTML += element;
			return valid
		}
	} else{
		document.getElementById('addReviewErrorsBlock').innerHTML = '';
